<template>
  <div>
    ...
    <van-field
      ...
      style="border:1px; border-color: black; background-color:aliceblue;"
    />
    <van-button round block type="primary"  @click="orderPackage">
            订购
    </van-button> 
  </div>
...
//定义一个函数来处理套餐查询
  const getPackageList = () => {
    ...
    console.log(iccid);
    axios.get(`${DOMAIN_RUL}/aihua/getCardFlowPackages`, {
      params: {
  ...
  const orderPackage = () => {
    ...
    axios.post(`${DOMAIN_RUL}/aihua/addCardPooledQuotaBatch`,formData)
    .then((response) => {
      console.log(response.data.result);
      // 处理返回的数据
